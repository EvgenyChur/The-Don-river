...
iPath = path_main + 'meteo_2000_2020/{}'.format(fileName)


# Считываем данные из модели SnoWE
#df_stat = pd.read_csv(iPath, skiprows = 0, sep=';', dayfirst = True, parse_dates = True, index_col = [0],
#                      skipinitialspace = True, na_values= ['9990','********'])

#Подготовка начальных данных для работы
df_stat = pd.read_csv(iPath, skiprows = 0, sep=';', dayfirst = True, parse_dates = True)
#print ('Columns:', df_stat.columns)


#Удаляем дубликаты и столбцы, которые не представляют интереса для данного исследования   
df_stat = df_stat.drop_duplicates(keep = False)
df_stat = df_stat.drop(['lat','lon','h_station','id_st','t2m_negative','hsnow'], axis=1)

#Создаем серии для заполнения пропусков в даннных
index_date = pd.to_datetime(df_stat['Date'])    # time step
ps = pd.Series(df_stat['ps'].values, index = index_date, dtype = 'float')           # air pressure at meteostation
pmsl = pd.Series(df_stat['pmsl'].values, index = index_date, dtype = 'float')         # air pressure at meteostation in according to see level
t2m = pd.Series(df_stat['t2m'].values, index = index_date, dtype = 'float')          # 2m air temperature
tmin2m = pd.Series(df_stat['tmin2m'].values, index = index_date, dtype = 'float')      # 2m min air temperature
tmax2m = pd.Series(df_stat['tmax2m'].values, index = index_date, dtype = 'float')      # 2m max air temperature   
tming = pd.Series(df_stat['tming'].values, index = index_date, dtype = 'float')       # min soil temperature for night 
td2m = pd.Series(df_stat['td2m'].values, index = index_date, dtype = 'float')         # 2m dew point
t_g = pd.Series(df_stat['t_g'].values, index = index_date, dtype = 'float')         # temperatura of soil
dd10m = pd.Series(df_stat['dd10m'].values, index = index_date, dtype = 'float')        # direction of wind
ff10mean = pd.Series(df_stat['ff10meam'].values, index = index_date, dtype = 'float')       # speed of wind
ff10max = pd.Series(df_stat['ff10max'].values, index = index_date, dtype = 'float')       # speed of wind
hsnow = pd.Series(df_stat['hsnow_snowe'].values, index = index_date, dtype = 'float')       # Depth of snow
swe = pd.Series(df_stat['swe'].values, index = index_date, dtype = 'float')       # Depth of snow
rho = pd.Series(df_stat['rho'].values, index = index_date, dtype = 'float')       # Depth of snow

# В случае, если рассматриваем для одной станции данные параметры не представляют интереса
#lat = pd.Series(df_stat['lat'].values, index = index_date, dtype = 'float')          # latitude of meteostation
#lon = pd.Series(df_stat['lon'].values, index = index_date, dtype = 'float')          # longitude of meteostation    
#h_station = pd.Series(df_stat['h_station'].values, index = index_date, dtype = 'float')    # higth of meteostation
...
# В случае, если рассматриваем для одной станции данные параметры не представляют интереса
#lat = nan_values_correction(lat)
#lon = nan_values_correction(lon)
#h_station = nan_values_correction(h_station)

# Отбрасываем строки с пустыми значениям, где заполненных параметров меньше 15
df_stat = df_stat.dropna(axis='rows', thresh=15)

# Выполняем переиндексирование по дате
df_stat['Date'] =  pd.to_datetime(df_stat['Date'], format='%Y-%m-%d')

# Устанавливаем новый индекс для массива данных
df_stat = df_stat.set_index('Date')

# Заполняем оставшиеся пустые значения в столбцах средним значением по столбцу
df_stat = df_stat.fillna(df_stat.mean())
...
for tr in range(w):
    try:
        y_w_1 = periods_winter[tr][0]
        y_w_2 = periods_winter[tr][1]
        df_stat_test = df_stat['swe'][y_w_1:y_w_2]
        
        if len(swe_test)>0:
            swe_test = pd.concat([swe_test, df_stat_test])
        else:
            swe_test = df_stat_test
...
#df_stat1 = df_stat.drop(['Date'], axis=1)

#df_des_stat1 = df_stat.describe()
#print ('Описательная статистика после коррекции', df_des_stat1)
 
#df_des_stat1.to_csv(iPath_exit + fileName[0:5] +'st.csv', sep=';', float_format='%.3f')
