...
# Создание переменных для создания дополнительных делений на оси абцисс и ординат для различных переменных
...
# Функция построения комплексного графики с выделением различными цвета твердых и жидких осадков
...
# Функция для отрисовки комплексного графика с использованием данных COSMO-Ru на основе данных SnoWE
...
"""
Функция для построения графика с двумя переменными: Функция строит диаграмму
pr_1 - основной расчетная переменная, далее по ней будет строиться график (нужно чтобы был индекс и значение).
индекс в формате даты с суточной дискретностью
prr_1 - вторая переменная
na_1 - текст легенды для основной расчетной переменной
n_2 - подпись заголовка для графика
n_3 - подпись оси y
pr_2, pr_3, pr_4 - пределы по шкале y (pr_6 - нижняя отметка, pr_7 - верхняя отметка, pr_8 - шаг)
...
# Блок 1. Подготовка исходной информации о снежном покрове по данным маршрутных снегомерных наблюдений (используем 1 раз, потом можно комментировать весь блок
...
# Блок 2. Отрисовка комплексного графика
